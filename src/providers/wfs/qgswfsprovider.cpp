...
void QgsWFSProvider::issueInitialGetFeature( bool force )
{
  ...
  const auto GetGeometryTypeFromOneFeature = [&]( bool includeBbox, bool force ) {
  ...
  const auto TryToDetectGeometryType = [&]() {
    ...
    GetGeometryTypeFromOneFeature( false, force );
    if ( mShared->mWKBType == Qgis::WkbType::Unknown )
    {
      ...
      if ( mShared->mWKBType == Qgis::WkbType::Unknown && !mSubsetString.isEmpty() )
      {
        ...
        if ( mShared->mWKBType == Qgis::WkbType::Unknown )
          GetGeometryTypeFromOneFeature( true, force );
...
void QgsWFSProvider::featureReceivedAnalyzeOneFeature( QVector<QgsFeatureUniqueIdPair> list, bool force )
{
  const Qgis::WkbType initialWkbType { mShared->mWKBType };
  if ( list.size() != 0 && ( force || initialWkbType == Qgis::WkbType::Unknown ) )
  {
    QgsFeature feat = list[0].first;
    QgsGeometry geometry = feat.geometry();
    if ( geometry.isNull() && initialWkbType == Qgis::WkbType::Unknown )
    {
      mShared->mWKBType = Qgis::WkbType::NoGeometry;
    }
    else
